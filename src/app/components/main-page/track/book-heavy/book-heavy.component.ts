...
@Component({
  selector: 'app-book-heavy',
  templateUrl: './book-heavy.component.html',
  styleUrls: ['./book-heavy.component.css']
})
export class BookHeavyComponent implements OnInit {
...
calcRoute() {
  ...
  directionsService.route(request, (result: { routes: { legs: { distance: any; duration: { text: any; }; }[]; }[]; }, status: any) => {
    if (status === google.maps.DirectionsStatus.OK) {
      ...
      this.http.post('https://inspiring-being-405110.el.r.appspot.com/mail/sendmail/third/fulltruck4', {
                email: email,
                number: number,
                name:name,
