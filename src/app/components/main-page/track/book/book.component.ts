...
initAutocomplete() {
  ...
  const autocomplete1 = new google.maps.places.Autocomplete(input1);
    // Replace 'yourBounds' with the actual bounds you want to restrict the search to
...
console.log('Final Rate:', rate);
      ...
      output.innerHTML =
        `<div class="alert-info">
           ...
           Rate: ${rate < 500 ? 500 : rate} rupees
         </div>`;
      directionsDisplay.setDirections(result);

      this.http.post('https://inspiring-being-405110.el.r.appspot.com/mail/sendmail/third/fulltruck1', {
                email: email,
                number: number,
                name:name,
